...
class ODEFunc(nn.Module):
    ...
    def __init__(self, feature_dim, temporal_dim, output_dimension):
        super(ODEFunc, self).__init__()
        torch.tensor(output_dimension, dtype=torch.int8)
        self.output_dimension = output_dimension
        self.x0 = None
        self.alpha = nn.Parameter(0.8 * torch.ones(output_dimension))
        self.beta = 0.6
    ...
    def forward(self, t, x):
        ...
        xa = torch.einsum('ij, kjlm->kilm', self.output_dimension, x)
...
class ODEG(nn.Module):
    def __init__(self, feature_dim, temporal_dim, output_dimension, time):
        super(ODEG, self).__init__()
        self.odeblock = ODEblock(ODEFunc(feature_dim, temporal_dim, output_dimension), t=torch.tensor([0, time]))
